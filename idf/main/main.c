...
// Phase 10: once WiFi is up, authenticate with Radiko (auth1 + auth2). The
// token authorises the stream in Phase 12; the area drives program info in
// Phase 14.
static void radiko_auth_task(void *arg) {
  ui_splash_status("Connecting to WiFi...");
  while (wifi_get_state() != WIFI_CONNECTED)
  ...
  radiko_auth_t auth;
  esp_err_t err;
  for (int attempt = 1; (err = radiko_authenticate(&auth)) != ESP_OK;
       attempt++) {
    int delay_ms = attempt * 3000;
    if (delay_ms > 30000)
      delay_ms = 30000;
    ESP_LOGW(TAG, "Radiko auth failed (attempt %d); retrying in %d s", attempt,
             delay_ms / 1000);
    char s[48];
...
void app_main(void) {
  ...
  if (psram == 0) {
    ESP_LOGE(TAG,
             "PSRAM not detected! Check board / sdkconfig (SPIRAM_MODE_OCT).");
  } else {
  ...
  // Phase 7: load persistent settings (before UI so it starts from saved
  // state).
  ESP_ERROR_CHECK(settings_init());

  // Phase 30: nationwide station DB + mmap the logo partition, then filter to
  // the saved listening area. Must precede ui_init, which sizes the player and
  // list from the active station count.
  stations_init();
  stations_set_area(settings_get()->area);
  ...
  // Phase 10: authenticate with Radiko (as the saved area — set above) once
  // WiFi is up.
  ...
  while (true) {
    ...
    ESP_LOGI(
        TAG,
        "alive (%" PRIu32 "), free internal %u KB (largest %u), PSRAM %u KB",
        ++tick, (unsigned)(heap_caps_get_free_size(MALLOC_CAP_INTERNAL) / 1024),
