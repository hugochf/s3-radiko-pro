...
- **タイムフリー(過去 7 日間の聴き逃し再生)** — 放送局リストを **PAST** に切り替えて局を選ぶと、その局の番組表を日付ごとに表示(‹ › で 1 週間前まで移動)。番組をタップすると Radiko から直接ストリーミング再生します(ダウンロードや SD カードへの保存は行いません)。シークバーのドラッグにも対応。**放送中の番組**もリストに残り、放送の進行に合わせてそのまま再生を続けます(シーク範囲は Radiko が配信済みの位置までに制限)。
