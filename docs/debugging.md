...
Likewise the nationwide station logos live in the `logos` partition (~2.7 MB,
generated by `tools/gen_stationdb.py`). Flash once after an erase/table change:

```sh
parttool.py -p /dev/cu.usbmodem2101 write_partition \
    --partition-name=logos --input components/stations/logos.bin
```

A blank `logos` partition just shows garbage/blank logos; stations still work.
